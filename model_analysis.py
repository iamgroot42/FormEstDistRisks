...
l2_adv_senses   = process_neurons("/p/adversarialml/as9rw/binary_stats/l2/delta_l2.txt", "/p/adversarialml/as9rw/binary_stats/l2/stats", threshold=threshold)
...
filtered_df = df[(df.Natural <= threshold) & (df['L-1'] <= threshold) \
				& (df['L-1'] <= threshold) & (df['a_L-1'] <= threshold) \
				& (df['L-2'] <= threshold) & (df['a_L-2'] <= threshold) \
