	...
	for i, (x, y) in tqdm(enumerate(dataloader), total=len(dataloader)):
		...
		for idx, j in enumerate(indices_):
			...
			if start + j > indices[trace]:
				...
				continue
			if start + j == indices[trace]:
				image = Image.fromarray((255 * np.transpose(cropped_faces[idx], (1, 2, 0))).astype('uint8'))
				file_prefix = "".join(str(c) for c in y[j].numpy())
				if y_[j] == 0:
					image.save(os.path.join(path, "0", file_prefix + "_" + str(trace)) + ".png")
				else:
					image.save(os.path.join(path, "1", file_prefix + "_" + str(trace)) + ".png")
...
if __name__ == "__main__":
	...
	splits_tr = [np.load(basepath + "splits_tr_1.npy"), np.load(basepath + "splits_tr_2.npy")]
	splits_te = [np.load(basepath + "splits_te_1.npy"), np.load(basepath + "splits_te_2.npy")]
	...
	for i in range(2):
		...
		print("Original label balance:", np.mean(labels_tr[splits_tr[i], target_prop]))
		...
		print("Filtered data label balance:", np.mean(labels_tr[actual_picked_indices_tr, target_prop]))
		...
		dump_files(os.path.join(paths[i], "test"),  model, testloader,  actual_picked_indices_te, target_prop)
		# dump_files(os.path.join(paths[i], "test"),  model, testloader,  picked_indices_te, target_prop)
		# Save train data
		os.mkdir(os.path.join(paths[i],   "train"))
		dump_files(os.path.join(paths[i], "train"), model, trainloader, actual_picked_indices_tr, target_prop)
