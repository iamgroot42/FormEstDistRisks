...
def find_impostors(model, delta_values, ds, images, labels, mean, std,
	...
	if corr_analysis:
		...
		# Pick easiest-to-attack neurons per image (AM)
		# Get scaled senses
		# scaled_delta_values = utils.scaled_values(delta_values, mean, std, eps=0)
		# easiest = np.argsort(np.abs(scaled_delta_values), axis=0)
		# Proper scaling (do not involve mean)
		easiest = np.argsort(np.abs(delta_values) / np.expand_dims(std, 1), axis=0)
		# No scaling
		# easiest = np.argsort(np.abs(delta_values))
		# Randomply shuffle
		# easiest = np.argsort(delta_values)
		# for j in range(easiest.shape[1]):
		# 	tp = easiest[:, j].T
	...
	impostors = parallel_impostor(model, delta_vec.cuda(), real, labels_, indices_mask.cuda(),
...
def parallel_impostor(model, delta_vec, im, labels, indices_mask, optim_type, verbose, eps,
...
if __name__ == "__main__":
	...
	senses = utils.get_sensitivities("./generic_deltas_%s/%d.txt" %( model_type, inject))
