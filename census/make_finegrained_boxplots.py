...
if __name__ == "__main__":
    ...
    # Custom legend
    if args.legend:
        meta_patch = mpatches.Patch(
            color='C0', label=r'$Acc_{meta-classifier}$')
        baseline_patch = mpatches.Patch(color='C1', label=r'$Acc_{baseline}$')
        threshold_patch = mpatches.Patch(
            color='C2', label=r'$Acc_{threshold}$')
    ...
    # Save plot
    sns_plot.figure.savefig("./meta_boxplot_finegrained_%s.png" % args.filter)
