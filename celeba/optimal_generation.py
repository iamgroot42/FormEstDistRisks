...
def gen_optimal(models, labels, sample_shape, n_samples, n_steps, step_size, latent_focus):
    ...
    for i in iterator:
        ...
        reprs = ch.stack([m(x_rand, latent=latent_focus) for m in models], 0)
        ...
        loss = ch.mean((reprs_z) ** 2) + ch.mean((2 - reprs_o) ** 2)
...
def get_all_models(dir):
    models = []
    files = os.listdir(dir)[:50]
    for pth in tqdm(files):
        m = get_model(os.path.join(dir, pth))
        models.append(m)
    return models
...
def main(args):
    ...
    X_train_1 = get_all_models(train_dir_1)
    X_train_2 = get_all_models(train_dir_2)
    Y_train = [0.] * len(X_train_1) + [1.] * len(X_train_2)
    Y_train = ch.from_numpy(np.array(Y_train)).cuda()

    X_test_1 = get_all_models(test_dir_1)
    X_test_2 = get_all_models(test_dir_2)
    ...
    reprs_0 = ch.stack([m(x_opt, latent=args.latent_focus)
                        for m in X_train_1], 0)
    reprs_1 = ch.stack([m(x_opt, latent=args.latent_focus)
    ...
    # Plot differences
    plt.plot(np.arange(len(X_train_1)), sorted(reprs_0))
    plt.plot(np.arange(len(X_train_2)), sorted(reprs_1))
    ...
    reprs_1 = ch.mean(1. * ch.sum(reprs_1 > 0, 2), 1).cpu().numpy()
    # Plot differences
    plt.plot(np.arange(len(X_test_1)), sorted(reprs_0))
    plt.plot(np.arange(len(X_test_2)), sorted(reprs_1))
