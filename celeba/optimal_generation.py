...
def ordered_samples(models_0, models_1, loader, args):
    ...
    diff_ids = np.argsort(-diffs)[:args.n_samples]
...
def get_all_models(dir, n_models, latent_focus):
    models = []
    files = os.listdir(dir)[:n_models]
    for pth in tqdm(files):
        m = get_model(os.path.join(dir, pth), fake_relu=True, latent_focus=latent_focus)
        models.append(m)
    return models
...
def main(args):
    ...
    X_train_1 = get_all_models(train_dir_1, args.n_models, args.latent_focus)
    X_train_2 = get_all_models(train_dir_2, args.n_models, args.latent_focus)
    Y_train = [0.] * len(X_train_1) + [1.] * len(X_train_2)
    Y_train = ch.from_numpy(np.array(Y_train)).cuda()

    # X_test_1 = get_all_models(test_dir_1, args.n_models, args.latent_focus)
    # X_test_2 = get_all_models(test_dir_2, args.n_models, args.latent_focus)
    X_test_1 = get_all_models(train_dir_1, 3 * args.n_models, args.latent_focus)
    X_test_2 = get_all_models(train_dir_2, 3 * args.n_models, args.latent_focus)
